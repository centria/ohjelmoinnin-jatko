...
* Osa 1: Use regular expressions in **DayOfWeek(string str)**, which returns true if the parameter string is an abbreviation of a day of the week (mon, tue, wed, thu, fri, sat, sun)

* Osa 2: Use regular expressions in **AllVowels(string str)**, which returns true if all the letters in given string are wovels.
...
* Osa 3: Use regular expressions in **TimeOfDay(string str)**  to check whether the parameter string expresses a time of day in the form hh:mm:ss (hours, minutes, and seconds each always take up two spaces).
